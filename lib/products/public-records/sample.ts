...
/**
 * Sensible defaults so a client can submit in one click.
 * No `cases[]` — the API evaluates every offense in the XML (simplest first path).
 */
export const SAMPLE_FORM: PublicRecordsFormInput = {
  searchId: '55788321',
  searchDate: '2026-04-10',
  orderId: '72849305',
  orderNumber: '72849305.1',
  applicantState: 'OR',
  candidate: {
    first_name: 'Marcus',
    middle_name: 'R',
...
export function buildEvaluateRequest(form: PublicRecordsFormInput): EvaluateRequest {
  return {
    record: {
      search_id: form.searchId.trim(),
      search_date: form.searchDate,
      order_id: form.orderId.trim(),
      order_number: form.orderNumber.trim(),
      applicant_state: form.applicantState.trim().toUpperCase() || undefined,
